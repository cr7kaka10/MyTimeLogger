...
from app.core.logic import MyTimeLoggerLogic
from app.core.hotkeys import HotkeyManager
from .dialogs import MarkdownInputDialog
from app.models.category_manager import CategoryManager
...
class MyTimeLoggerGUI(QWidget):
    ...
    def __init__(self, config):
        ...
        self.setWindowIcon(QIcon(resource_path(os.path.join("assets", "icons", "icon.ico"))))
    ...
    def rebuild_layout(self):
        ...
        if not hasattr(self, 'expand_btn'):
            ...
            self.expand_btn.setIcon(QIcon(resource_path("assets/icons/expand.svg")))
    ...
    def create_tray_icon(self):
        """创建系统托盘图标和菜单"""
        self.tray_icon = QIcon(resource_path(os.path.join('assets', 'icons', 'icon.ico')))
    ...
    def switch_ui_mode(self, to_mini: bool):
        """
        统一切换 UI 模式：大面板 (完全互斥)
        注：Mini 模式已废弃，to_mini=True 相当于隐藏大面板进入托盘后台。
        """
        full_win = self._ensure_activity_panel_window()
        self.is_mini_mode = to_mini
        if to_mini:
            full_win.hide()
            self.hide()
        else:
    ...
    def _on_stats_ready(self, rows, open_browser):
        ...
        html_path = resource_path(os.path.join("local_data", "statistics.html"))
        week_map = {
