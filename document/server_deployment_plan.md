...
### 2.2 `cloud_sleep_store.py`

云端 Buffer SQLite 存储层，默认数据库为 `cloud_sleep_jobs.db`。
...
该脚本会读取 `.env` 的 `SLEEP_AUTH_TOKEN`，并把本地 `config.json` 的 `cloud_sleep_sync` 指向 `http://127.0.0.1:8000`。
...
uvicorn cloud_sleep_api:app --host 0.0.0.0 --port 8000
```
...
推荐：
- Python 3.10+
- `uvicorn cloud_sleep_api:app --host 127.0.0.1 --port 8000`
...
- `cloud_attachments/` 定期清理。
- `cloud_sleep_jobs.db` 定期备份。
