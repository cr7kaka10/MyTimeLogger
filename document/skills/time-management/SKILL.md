...
3. **数据自动缓存**：aTimeLogger 获取到的日数据会自动保存到本地 SQLite `atm` 表中，减少重复 API 请求并提升分析速度。
4. **入睡/起床时间计算需要同时有 aTimeLogger 和华为健康数据**
5. **报告生成后会自动保存到 reports/ 目录**，并会在界面的 Markdown 区域展示
6. **时间格式处理**：注意跨天情况（如23:00入睡，07:00起床）
