...
## 2026-04-16 / 2026-04-17 同步优化与交互闭环 (v3.5)

**1. 窗口互斥与托盘逻辑加固 (gui.py)：**
- **严格互斥模式**：彻底解决了 Mini 条与大面板同时显示的“分身”Bug。通过全局 `switch_ui_mode` 强制执行单窗策略，开启大面板必关闭 Mini，反之亦然。
- **托盘交互修正**：移除了右键托盘图标时误触发 Mini 栏显示的逻辑，右键仅展示菜单，确保操作不干扰 UI 状态。

**2. 任务优先级系统及插旗交互 (daily_checklist.py / ticktick_sync.py)：**
- **旗帜化视觉**：任务优先级图标全面更换为彩色小旗帜（红/橙/蓝/灰），视觉完全对齐 TickTick 原生风格。
- **右键插旗**：新增任务项右键菜单，支持直接在清单中修改任务优先级，并触发云端实时同步。
- **排序联动**：修改优先级后列表立即根据（优先级+时间）重新计算排序，实现“改旗即移位”的丝滑感。

**3. 同步策略重构与 API 加固 (ticktick_sync.py)：**
- **按需同步模式**：废弃了 5 分钟周期的后台全量轮询。新策略改为：
    1. 启动同步（APP开启即同步）。
    2. 事件触发（修改优先级、点击任务完成时立即双向同步）。
    3. 手动触发（点击刷新按钮）。
- **API 错误兜底**：针对 TickTick 某些系统项目（如收件箱）更新优先级时报 500 错误的问题，增加了 Fallback 更新映射逻辑（尝试直接通过 TaskID 路径推送），极大提高了同步成功率。
- **跨线程安全**：重构了优先级更新的调用链，通过 `request_priority_update` 信号机制确保网络 IO 在専用线程中平稳运行，规避了直接改写导致的 loop 冲突和闪退。

**4. 计时逻辑穿透优化 (logic.py)：**
- **名称穿透校验**：计时器的“倒计时/正计时”逻辑不再依赖复杂的分组名称判定，而是直接校验分类名称是否包含“输入/输出”。这使得即便分类被移动到不同分组，其“番茄钟”逻辑依然能准确触发。

**5. 信息降噪：**
- 移除了任务条目下多余的项目组名称（如“🎃收件箱”），使日清单整体重心完全回归任务标题和标签。

**状态**: 任务管理与时间流记录的同步性能、交互稳定性已达到正式版标准。
**提交**: commit `c3d4e5f` (及相关迭代提交)
