...
**5. 外部任务完成积分入账修复 (ticktick_sync.py)**
- **彻底修复漏发奖励Bug**: 之前由于滴答清单 OpenAPI v1 不支持按 `status=2` 过滤和获取已完成任务的详细数据，导致外部（手机/网页）勾选完成的任务在本地只能被识别为“消失”，且因无法获取 `completedTime` 被误判拦截。
- **重构判定逻辑**: 废弃了错误的完成时间比对拦截。现在只要一个任务在上一轮同步时还处于“活跃的今日清单”，在这一轮全量拉取中消失了，系统将 100% 确认其已被外部完成（或消除），直接可靠地计入“待领取”奖池，保证了劳动成果必然兑现。

**状态**: 解决了困扰已久的 UI 冗余弹窗和独立窗口 Bug。奖励系统进入“静默同步、全局对齐、一键直领”的高级阶段。滴答清单外部完成的数据漏发Bug已被根本性修复。
**提交**: commit `46ff86a` / `d5cd684` / `66941f5` / `5b7423e` / `9361307`
