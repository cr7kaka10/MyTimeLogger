# MyTimeLogger v0.96 (沉浸式学习记录器)

## 源起与理念

+ 本项目由github项目https://github.com/Yogioo/StudyTimer#改进而来，感谢原作者
+ version 0.96做了以下改进
  + 增加大专注完成的音频
  + 暂停和恢复按钮合并成一个，开始按钮单独设置
  + 暂停会弹框提示输入原因
  + 大专注完成会弹框提示输入专注时段做了哪些事
  + 随机休息间隔可以设置3~5、5~7、6~9，大专注时长可以设置30min、60min、90min
  + 增加统计功能，托盘区图标右键，点统计可以查看专注记录
  + 添加Sqlite数据库支持
...
*   📊 **统计报表**: **(v0.96 优化)** 采用 SQLite 数据库持久化存储所有专注数据。支持在托盘菜单一键生成卡片式 HTML 统计报表，直观查看历史记录。
...
1.  **下载**: 获取最新的 `MyTimeLogger_v0.96.exe`（单文件版本）。
...
| 操作 | 快捷键 | 功能说明 |
| :--- | :--- | :--- |
| **开始 / 继续** | `Alt` + `Z` | 启动计时器或从暂停状态恢复计时 |
| **暂停 / 切换** | `Alt` + `C` | 暂停当前计时，会触发原因输入弹窗 |
...
```text
MyTimeLogger/
├── my_time_logger.py        # 主程序逻辑 (PyQt6 + Pygame)
