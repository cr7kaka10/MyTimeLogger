...
+ **version 0.99 核心改进**
  + **迷你模式 (Mini Mode)**: 支持在垂直常规模式与水平迷你模式（高度仅 40px）间一键切换，完美适配 Windows 任务栏。
  + **高精 SVG 图标**: 引入原生 SVG 图标支持，极致透明感（30% 不透明度），视觉审美大幅提升。
  + **布局与样式加固**: 移除了自由缩助手柄，采用固定尺寸设计，彻底解决布局错位问题。
+ version 0.98 及以前早期功能
  + 增加大专注完成的音频及弹框记录
  + 随机休息间隔设置与可视化统计功能
  + Sqlite 数据库支持，保障数据持久化
